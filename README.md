...
- **Manual transcript annotations**: While recording or on a saved meeting, paste an image from the clipboard (Ctrl+V or a paste button) or add a text note/marker, each anchored to a point in the transcript timeline. Annotations render inline with the transcript and persist with the meeting.
- **Meeting export (Markdown + PDF)**: Export a meeting to a portable `.md` or `.pdf` file, or copy it as Markdown. The export includes the title, the AI summary, and the full transcript with notes, markers, and pasted images interleaved by time; images are embedded, and tables and rich formatting from the summary are preserved in the PDF.
- **Live per-segment translation**: An optional toggle translates each finalized transcript segment during recording and shows the translation alongside the original, persisted with the meeting. The engine is configurable between a local Ollama model and a self-hosted LibreTranslate service, with a target-language picker (default English → Brazilian Portuguese).
